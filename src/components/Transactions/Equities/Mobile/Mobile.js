...
const buildRows = (rows, loading) => {
  ...
  return rows.map((b, i) => (
    <AssetWrapper key={`mobileEquities_${i}`}>
      <AssetRow>
...
function Mobile({
  ...
  const totaltransactions = generateSum(equityIncomeTransactions);
  return (
    <Fragment>
      ...
      {!isEmpty && (
        <Wrapper>
          ...
          {buildRows(equityIncomeTransactions, loading)}
        </Wrapper>
      )}
      {!isEmpty && (
        <Pagination
      ...
      {!isEmpty && !loading && (
        <LinkCardWrapper>
          <LinkCard
            iconType="FiLoader"
            to="/investments/overview?menu=movements&from=equities"
