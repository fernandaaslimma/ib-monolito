...
function Mobile({
  ...
  const totaltransactions = generateSum(fixedIncomeTransactions);
  return (
    <Fragment>
      ...
      {!isEmpty && (
        <Wrapper>
          ...
          {buildRows(fixedIncomeTransactions, loading)}
        </Wrapper>
      )}
      {!isEmpty && (
        <Pagination
      ...
      {!isEmpty && !loading && (
        <LinkCardWrapper>
          <LinkCard
            iconType="FiLoader"
            to="/investments/overview?menu=movements&from=fixedIncome"
